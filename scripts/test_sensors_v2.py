...
def setup_door(scene: InteractiveScene):
    """Setup the door."""
    # scene["door"].data.body_pos_w = torch.zeros_like(scene["door"].data.body_pos_w)
    print(scene["door"].data.body_pos_w)
    scene["door"].set_joint_position_target(scene["door"].data.soft_joint_pos_limits[..., 0])
